  ...
  <v-app>
    ...
    <v-container style="margin-top: 75px; max-width: 1000px">
      ...
      <span v-if="this.allowedTags.length > 1">
        ...
        <v-menu offset-y>
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-bind="attrs" v-on="on" class="tag-button">
              {{ selectedTag }}
            </v-btn>
          </template>
          <v-list>
            <v-list-item @click="updateSelectedTag('all')">
              <v-list-item-title>all</v-list-item-title>
            </v-list-item>
            <v-list-item v-for="tag in this.allowedTags" :key="tag" @click="updateSelectedTag(tag)">
      ...
      <div class="events-container">
        <AdminEventCard
          v-for="event of acmEvents.filter(e => (this.selectedTag == 'all' || e.tags?.includes(this.selectedTag) || false))"
